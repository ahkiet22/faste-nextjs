...
type TCheckoutState = {
  totalPrice: number
  productsSelected: { product: string; amount: number }[]
}
...
const CheckoutProductPage: NextPage<TProps> = () => {
  ...
  const memoQueryProduct = useMemo(() => {
    ...
    const data: any = router.query.state
    if (data) {
      const states: TCheckoutState = decrypt(data) as TCheckoutState
      result.totalPrice = states.totalPrice || 0
      result.productsSelected = states.productsSelected ? handleFormatDataProduct(states.productsSelected) : []
    }
  ...
  useEffect(() => {
    const dataQuery = router.query.state
    if (dataQuery) {
      const data: any = decrypt(dataQuery as string)
                ...
                return (
                  <Fragment key={item.product}>
                    <Box sx={{ display: 'flex', alignItems: 'flex-start', gap: '8px' }}>
                      <Avatar sx={{ width: '100px', height: '100px' }} variant='rounded' src={item.image} />
                      <Typography
                        sx={{
                          fontSize: '20px',
                          flexBasis: '35%',
        ...
        <Box sx={{ display: 'flex', alignItems: 'flex-end', flexDirection: 'column', gap: 2 }}>
          <Box sx={{ display: 'flex', gap: '2px' }}>
            ...
            <Typography sx={{ fontSize: '20px', width: '200px' }}>
              {formatNumberToLocal(memoQueryProduct.totalPrice, { language: (i18n.language as 'vi') || 'en' })}
            </Typography>
          </Box>
          <Box sx={{ display: 'flex', gap: '2px' }}>
            <Typography sx={{ fontSize: '20px', width: '200px' }}>{t('Price_shipping')}:</Typography>
            <Typography sx={{ fontSize: '20px', width: '200px' }}>
              {formatNumberToLocal(memoPriceShipping, { language: (i18n.language as 'vi') || 'en' })}
            </Typography>
          </Box>
          <Box sx={{ display: 'flex', gap: '2px' }}>
            ...
            <Typography sx={{ fontSize: '20px', width: '200px', fontWeight: 600, color: theme.palette.primary.main }}>
              {formatNumberToLocal(+memoQueryProduct.totalPrice + +memoPriceShipping, {
                language: (i18n.language as 'vi') || 'en'
              })}
            </Typography>
          </Box>
