...
// ** Mui
import { Avatar, AvatarGroup, Box, Chip, ChipProps, Grid, Checkbox, Typography, styled, useTheme } from '@mui/material'
  ...
  const columns: GridColDef[] = [
        ...
        return (
          <AvatarGroup max={1}>
            {row.orderItems?.map((item: TItemProductMe) => {
              return <Avatar key={item?.product?._id} alt={item?.product?.slug} src={item?.image} variant='rounded' />
            })}
          </AvatarGroup>
        )
      }
    ...
    },
    {
      ...
      renderCell: params => {
        ...
        return (
          <Typography>{formatNumberToLocal(row.totalPrice, { language: i18n.language as 'vi' | 'en' })}</Typography>
        )
      }
    },
    {
